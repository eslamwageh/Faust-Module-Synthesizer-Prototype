 ...
 */
const createFaustNode = async (audioContext, dspName = "template", voices = 0,  module ="",sp = false, bufferSize = 512) => {
    ...
    /** @type {FaustDspMeta} */
    const dspMeta = await (await fetch(`./dsp-meta${module}.json`)).json();

    // Compile the DSP module from WebAssembly binary data
    const dspModule = await WebAssembly.compileStreaming(await fetch(`./dsp-module${module}.wasm`));
